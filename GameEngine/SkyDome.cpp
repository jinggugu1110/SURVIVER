...
SkyDome::SkyDome(ID3D11Device* device, ID3D11DeviceContext* deviceContext)
{
    ...
    DX::ThrowIfFailed(CreateDDSTextureFromFile(device, L"data\\sky\\cloud001.dds", resource.GetAddressOf(), m_textureCloud.ReleaseAndGetAddressOf()));
    DX::ThrowIfFailed(CreateDDSTextureFromFile(device, L"data\\sky\\perturb001.dds", resource.GetAddressOf(), m_texturePerturb.ReleaseAndGetAddressOf()));

    auto blob =     DX::ReadData(L"data\\shaders\\Cloud.cso");
...
void SkyDome::Render(ID3D11DeviceContext* deviceContext, const DirectX::SimpleMath::Matrix& world,
